...
$title = 'Editar oxigênio';

$id = (int)($_GET['id'] ?? 0);
$row = $id > 0 ? crud_find('oxigenio', 'idOxigenio', $id) : null;
if (!$row) {
    flash_set('err', 'Registro não encontrado.');
    redirect('/pages/oxigenio/index.php');
}

$quantidade = (string)$row['quantidadeAbastecida'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        try {
            $pdo = db();
            $stmt = $pdo->prepare('UPDATE oxigenio SET quantidadeAbastecida=:q, estado=:e WHERE idOxigenio=:id');
            $stmt->execute(['q' => (float)$quantidade, 'e' => $estado, 'id' => $id]);
            flash_set('ok', 'Registro atualizado.');
            redirect('/pages/oxigenio/index.php');
        } catch (Throwable $e) {
...
<div class="card">
  <h2>Editar registro de oxigênio</h2>
